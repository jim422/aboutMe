...
function serializeData(viewModel) {
	viewModel.serialize_data_list().forEach(function (serializeFn) {
		serializeFn()
	});
...
function ignoreUselessFields(viewModel) {
	...
	if (viewModel.data_screenshot_required() == 2) {
		fields = uselessFields.concat(['data_screenshot_limit', 'data_screenshot_examples', 'data_screenshot_record_items', 'data_screenshot_hogwarts_visibility'])
	}
